...
- NAT-PMP auto-starts when the conf comments include `NAT-PMP ... = on` (or `NAT_PMP_PORT` is set); gateway is Interface `DNS` (Proton: `10.2.0.1`).
- Isolated download-only hosts may use `WG_USE_WG_QUICK=1` / `WG_KILL_SWITCH=true` if they are not mesh hubs.

See also: [SECRET-ROTATION.md](SECRET-ROTATION.md), [MTLS-STAGING.md](MTLS-STAGING.md).
